...
双向链表：
一、创建思路：
    1.节点结构的改变：指针域需要有两个指针，一个指向下一个节点，另一个指向上一个节点
*/
// ============================================
// ========= 双向链表的创建与各种操作 ===========
// ============================================
...
typedef struct node{
    ...
    struct node *next, *prev; // 创建存储下一个节点内存地址的指针next和上一个节点内存地址的prev
}Node;
...
int insertHead(Node* L, ElemType e)
{
    ...
    p->next = L->next;  // 新节点p的指针域p->next指向下一个节点(即头节点L的指针域L->next)
    p->prev = L;        // 新节点p的指针域p->next指向上一个节点(即头节点)

    // 排除双向链表只有一个头节点的情形
    if (L->next != NULL)
    {
        L->next->prev = p; // 将第一个节点的prev指向新插入的节点p
    }
    L->next = p;
    return 1;
}
...
// 步骤2：开始尾插
Node* insertTail(Node* tail, ElemType e)   // tail是步骤1获取的尾节点
{
...
int deleteNode(Node* L, int pos) // L是头节点，pos是要删除的位置
{
    ...
    q->next->prev = p; // 让删除节点q的后继节点的prev指向q的前驱节点p
    free(q);           // 释放要删除节点的内存空间
    return 1;
}
